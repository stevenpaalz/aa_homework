...
class MetaCorgiSnacks
  ...
  def method_missing(name, *args)
    name = name.to_s
    :info_method = "get_#{name}_info()"
    :tastiness_method = "get_#{name}_tastiness()"
    
    info = args.first.send(info_method(args.second))
    tastiness = args.first.send(tastiness_method(args.second))
